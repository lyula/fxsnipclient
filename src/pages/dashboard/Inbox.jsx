  ...
  function groupMessagesByDate(messages) {
    ...
    messages.forEach((msg, index) => {
      ...
      groups.push({
        type: "msg",
        msg,
        isFirstInGroup: index === 0 || new Date(messages[index - 1].createdAt).getDate() !== msgDay,
      });
    });
    return groups;
  }
  ...
  if (selectedUser) {
    ...
    return (
      <div className="w-full max-w-lg mx-auto h-screen flex flex-col bg-white dark:bg-gray-800 rounded-none sm:rounded-xl shadow p-0">
        ...
        {/* Scrollable Messages */}
        <div
          className="flex-1 px-4 py-3 space-y-3 bg-gray-50 dark:bg-gray-900 overflow-y-auto no-scrollbar"
          style={{ minHeight: 0 }}
        >
          <style>
            {`
            ...
            {groupedMessages.map((item, idx) =>
              item.type === "date" ? (
                ...
                <div
                  ...
                  } ${item.isFirstInGroup ? "mt-4" : "mt-1"} mb-2`}
                >
                  <div
                  ...
                  >
                    {item.msg.text}
                    <div className="text-[10px] text-right mt-1 opacity-60 flex flex-col items-end">
                      <span>
                      ...
                      {/* Show read status for messages sent by me */}
                      {item.msg.from === myUserId && (
                        <span className="ml-1">
                          {item.msg.read ? (
        ...
        <form
          onSubmit={handleSend}
          className="flex items-center gap-2 px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 sticky bottom-0 z-30"
        ...
        {users.length === 0 ? (
          ...
          users.map(user => (
            ...
            >
              ...
              <div className="text-xs/tutorialspoint.com/reactjs/how_to_remove_the_scrollbar_from_the_react_material_ui_dialog.htm text-gray-400">{user.lastTime}</div>
            </button>
          ))
        )}
      </div>
