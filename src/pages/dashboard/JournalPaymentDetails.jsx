...
export default function JournalPaymentDetails() {
  const { id } = useParams();
  ...
  return (
    <div className="w-full max-w-xl mx-auto p-4" style={{ fontFamily: `system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Oxygen', 'Ubuntu', 'Cantarell', 'Fira Sans', 'Droid Sans', 'Helvetica Neue', Arial, sans-serif`, fontSize: 'inherit' }}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
        <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-gray-100">Journal Payment Details</h2>
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span className="font-semibold text-gray-700 dark:text-gray-200">M-Pesa Code:</span>
            <span className="font-mono text-sm text-gray-900 dark:text-gray-100">{payment.mpesaCode || payment.code || payment.receipt || payment.rawResponse?.MpesaReceiptNumber || payment.rawResponse?.receipt || "-"}</span>
          </div>
          <div className="flex items-center justify-between">
          ...
          {!isSuccess && (payment.failureReason || payment.rawResponse?.ResultDesc) && (
            <div className="flex items-center justify-between">
              <span className="font-semibold text-gray-700 dark:text-gray-200">Reason:</span>
              <span className="font-mono text-xs text-red-700 dark:text-red-300">{payment.failureReason || payment.rawResponse?.ResultDesc}</span>
            </div>
          )}
          <div className="flex items-center justify-between">
